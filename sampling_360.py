...
#nput_folder = '/TESIS/DATOS_1/rf_train'
...
#save_dir='D:/DiffusionModels/sampling_direccion'
save_dir = 'D:/DiffusionModels/direccion'
training_epochs = 360
model13A = UNETv13(residual=True, attention_res=[], group_norm=True).to(device)
# Intentamos cargar los pesos del modelo
model_dict = model13A.state_dict()
checkpoint = torch.load(f"{save_dir}/model_{training_epochs}.pth", map_location=device)

# Filtramos solo las claves que coinciden entre el modelo y el checkpoint
checkpoint_dict = {k: v for k, v in checkpoint.items() if k in model_dict and v.shape == model_dict[k].shape}

# Actualizamos el state_dict del modelo con los parámetros coincidentes
model_dict.update(checkpoint_dict)
model13A.load_state_dict(model_dict)
print("Num params: ", sum(p.numel() for p in model13A.parameters()))
"""
model13A.load_state_dict(torch.load(f"{save_dir}/model_{training_epochs}.pth", map_location=device), strict=False)
print("Num params: ", sum(p.numel() for p in model13A.parameters()))
